...
assert.equal(trustedOrganizationDomain.reason, 'functional_on_verified_organization_domain');

const verifiedFunctionalAlias = classifyContactCandidate({ email: 'brv-geschaeftsstelle@verband.example', context: 'Geschäftsstelle' }, 'https://verband.example/', { trustedFunctionalLocalParts: ['brv-geschaeftsstelle'] });
assert.equal(verifiedFunctionalAlias.governanceState, 'auto-approved-functional');
assert.equal(verifiedFunctionalAlias.reason, 'verified_functional_alias');

const externalGeneric = classifyContactCandidate({ email: 'info@fremde-domain.de', kind: 'functional', context: 'Allgemeiner Kontakt' }, clubSite);
assert.equal(externalGeneric.governanceState, 'review-functional');
assert.equal(externalGeneric.autoApproved, false);

const incorrectlyTaggedPersonal = classifyContactCandidate({ email: 'max.mustermann@beispiel-ruderverein.de', kind: 'functional', context: 'Ruderwart Max Mustermann', role: 'Ruderwart' }, clubSite);
...
const personalProvider = classifyContactCandidate({ email: 'max.mustermann@t-online.de', context: 'Ruderwart Max Mustermann', role: 'Ruderwart' }, clubSite);
...
assert.deepEqual(evaluateContactLifecycle({ verifiedAt: '2026-07-10T00:00:00.000Z', now }), { lifecycleState: 'fresh', ageDays: 31, directEligible: true });
assert.deepEqual(evaluateContactLifecycle({ verifiedAt: '2026-01-01T00:00:00.000Z', now }), { lifecycleState: 'stale', ageDays: 221, directEligible: true });
assert.deepEqual(evaluateContactLifecycle({ verifiedAt: '2025-10-01T00:00:00.000Z', now }), { lifecycleState: 'expired', ageDays: 313, directEligible: false });
...
const eligible = evaluateSnapshotEligibility({ email: 'info@beispiel-ruderverein.de', sourceUrl: 'https://www.beispiel-ruderverein.de/kontakt', verifiedAt: '2026-07-10T00:00:00.000Z' }, clubSite, { now });
assert.equal(eligible.snapshotEligible, true);
assert.equal(eligible.governanceState, 'auto-approved-functional');

const noProvenance = evaluateSnapshotEligibility({ email: 'info@beispiel-ruderverein.de' }, clubSite, { now });
...
const suppressed = evaluateSnapshotEligibility({ email: 'info@beispiel-ruderverein.de', sourceUrl: 'https://www.beispiel-ruderverein.de/kontakt', verifiedAt: '2026-07-10T00:00:00.000Z' }, clubSite, { now, suppressed: true });
assert.equal(suppressed.snapshotEligible, false);
assert.equal(suppressed.governanceState, 'suppressed');

const personal = evaluateSnapshotEligibility({ email: 'max.mustermann@beispiel-ruderverein.de', sourceUrl: 'https://www.beispiel-ruderverein.de/vorstand', verifiedAt: '2026-07-10T00:00:00.000Z', role: 'Ruderwart' }, clubSite, { now });
