...
function nearestUniqueLrvContext($, element, lrvs) {
  ...
  for (let depth = 0; depth < 7 && current.length; depth += 1) {
    ...
    if (text && text.length <= 2000) {
      ...
      if (matches.length === 1) return { record: matches[0], context: text };
    }
...
export function extractLaenderratCandidates(html, lrvs, verifiedAt = new Date().toISOString()) {
  ...
  for (const element of $('a[href^="mailto:"]').toArray()) {
    ...
    if (!email) continue;
    const resolved = nearestUniqueLrvContext($, element, lrvs);
...
function publicReport(candidates, lrvCount, generatedAt) {
  ...
  for (const candidate of candidates) {
    const row = byOrganization.get(candidate.organizationId) || { candidates: 0, autoApproved: 0, review: 0 };
    row.candidates += 1;
...
function markdown(report) {
  const rows = report.organizations.map((item) => `| ${item.organizationId} | ${item.candidates} | ${item.autoApproved} | ${item.review} |`).join('\n');
  return `# DRV-Länderrat – LRV Contact Enrichment\n\nStand: ${report.generatedAt}\n\n- Registry-LRV: **${report.lrvRegistryCount}**\n- eindeutig zugeordnete LRV: **${report.matchedOrganizations}**\n- Kontaktkandidaten: **${report.candidateCount}**\n- davon Policy-Auto-Approved: **${report.autoApprovedCandidateCount}**\n- Review: **${report.reviewCandidateCount}**\n\n| DRV-ID | Kandidaten | Auto-Approved | Review |\n| --- | ---: | ---: | ---: |\n${rows}\n\nDer öffentliche Report enthält keine E-Mail-Adressen. Der Länderrat wird nur als offizielle DRV-Zweitquelle genutzt; persönliche Adressen bleiben Review.\n`;
}
...
async function main() {
  ...
  console.log(`[lrv-laenderrat] matched=${report.matchedOrganizations}; candidates=${report.candidateCount}; approved=${report.autoApprovedCandidateCount}; review=${report.reviewCandidateCount}`);
}
